...
def asr(
    task : Union[str, None] = "transcriptions",
    language: Union[str, None] = Query(default=None, enum=LANGUAGE_CODES),
    prompt: Union[str, None] = Query(default=None),
    audio_file: UploadFile = File(...),
    encode : bool = Query(default=True, description="Encode audio first through ffmpeg"),
    response_format : Union[str, None] = Query(default="txt", enum=["txt", "vtt", "srt", "tsv", "json"]),
    ...
    result = transcribe(load_audio(audio_file.file, encode), task, language, prompt, word_timestamps, response_format)
    return StreamingResponse(
        result, 
        media_type="text/plain", 
        headers={
                'Asr-Engine': ASR_ENGINE,
                'Content-Disposition': f'attachment; filename="{audio_file.filename}.{response_format}"'
            })


@app.post("/v1/audio/translations", tags=["Endpoints"])
def asr(
    task : Union[str, None] = "translations",
    # language: Union[str, None] = Query(default=None, enum=LANGUAGE_CODES),
    prompt: Union[str, None] = Query(default=None),
    audio_file: UploadFile = File(...),
    encode : bool = Query(default=True, description="Encode audio first through ffmpeg"),
    ...
    result = transcribe(load_audio(audio_file.file, encode), task, "", prompt, word_timestamps, response_format)
    return StreamingResponse(
        result, 
        media_type="text/plain", 
        headers={
